...
| quote-ids-filepath | The path to a JSON file containing a list of quote ids to search for over the entire corpora. This must be set if --no-search-quick-lookup is passed. If is is not set, or is set to the empty string, an error will be raised  | ""
